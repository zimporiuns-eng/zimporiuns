...
const traducoes: Record<Idioma, any> = {
  ...
  'en': {
    ...
    acessar: 'Visit',
    zimcoins: 'ZimCoins',
    dataCadastro: 'Member since',
    classificacao: 'Content Rating',
    rodape: 'Profile hosted on Zimporiuns — Brazilian Creators Directory',
    cta: 'Want your profile too? Subscribe to a plan!',
    livre: 'All Ages'
  },
  'es': {
    plano: 'Plan',
    ...
    acessar: 'Visitar',
    zimcoins: 'ZimCoins',
    dataCadastro: 'Miembro desde',
    classificacao: 'Clasificación Indicativa',
    rodape: 'Perfil alojado en Zimporiuns — Directorio de Creadores Brasileños',
    cta: '¿Quieres tu perfil también? ¡Suscríbete a un plan!',
    livre: 'Libre'
  },
  'pt-PT': {
    plano: 'Plano',
    ...
    cta: 'Quer o seu perfil também? Subscreva um plano!',
    livre: 'Livre'
  },
  'fr': {
    plano: 'Plan',
    ...
    acessar: 'Visiter',
    zimcoins: 'ZimCoins',
    dataCadastro: 'Membre depuis',
    classificacao: 'Classification Indicative',
    rodape: 'Profil hébergé sur Zimporiuns — Annuaire des Créateurs Brésiliens',
    cta: 'Vous voulez votre profil aussi ? Abonnez-vous à un plan !',
    livre: 'Tous publics'
  },
  'de': {
    plano: 'Plan',
    canais: 'Kanäle',
    seguidores: 'Abonnenten',
    acessar: 'Besuchen',
    zimcoins: 'ZimCoins',
    dataCadastro: 'Mitglied seit',
    classificacao: 'Alterseinstufung',
    rodape: 'Profil gehostet auf Zimporiuns — Brasilianisches Ersteller-Verzeichnis',
    cta: 'Willst du auch dein Profil? Abonniere einen Plan!',
    livre: 'Freigegeben'
  },
  'ja': {
    plano: 'プラン',
    ...
    zimcoins: 'ZimCoins',
    dataCadastro: '登録日',
    classificacao: 'コンテンツレーティング',
    rodape: 'Zimporiunsでホストされているプロフィール — ブラジルのクリエイターディレクトリ',
    cta: 'あなたもプロフィールを作りませんか？プランを購読しましょう！',
    livre: '全年齢'
  },
  'zh': {
    plano: '计划',
...
const criadorExemplo = {
  nome: 'Nome do Criador',
  plano: 'Bronze',
  descricao: 'Criador de conteúdo digital.',
  canais: [
...
export default function PerfilCriador() {
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      {/* CANAIS */}
      <h2 style={{ fontSize: '1.5rem', marginBottom: '20px', borderBottom: '1px solid #e0e0e0', paddingBottom: '10px' }}>
        🔗 {t.canais}
      </h2>
      <div style={{ display: 'grid', gap: '15px', marginBottom: '40px' }}>
        {criador.canais.map((canal, index) => (
          <div key={index} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 20px', backgroundColor: '#f9f9f9', borderRadius: '10px' }}>
            ...
            <a href={canal.url} target="_blank" rel="noopener noreferrer" style={{ backgroundColor: '#0070f3', color: 'white', padding: '8px 20px', borderRadius: '5px', textDecoration: 'none', fontSize: '0.9rem' }}>
              {t.acessar}
            </a>
          </div>
        ))}
